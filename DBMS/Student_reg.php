...
<div class="signup-form">
    <form action="Student_reg.php" method="post">
		...
		<div class="form-group">
        	<input type="number" class="form-control" title="Must be numerical" name="grade" placeholder="Grade" required="required">
        </div>
        <div class="form-group">
...
$sql =" insert into student_reg (fname,lname,email,contact,city,grade)values ('$fname','$lname','$emailid','$contact','$city','$grade')";


if(mysqli_query($dbname, $sql)){
    echo "Records inserted successfully in student_reg table.";
} else{
    echo "ERROR1: Could not able to execute $sql. " . mysqli_error($dbname);
}
 
 $sql1=" insert into student_login values((select S_id from student_reg where fname = '$fname'),'$username','$pass')";
 
  if(mysqli_query($dbname, $sql1)){
    echo "Records inserted successfully in student_login table";
} else{
 ...
 if(isset($_POST["subject"]))  
        { 
			...
			{
				$sql3=" insert into student_sub values((select S_id from student_reg where fname = '$fname'),(select Sub_id from subject where Sub_name='$subject'))";
